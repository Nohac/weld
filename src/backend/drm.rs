...
use bevy::{input::ButtonState as BevyButtonState, math::UVec2};
...
use crate::{
    AppArguments,
    raw_input::{
        InputPosition, LinuxButtonCode, LinuxKeycode, RawScrollFrame, RawScrollPhase,
...
pub(crate) fn run(arguments: AppArguments, signals: Signals) -> Result<()> {
    ...
    let mut input_adapter = StandaloneInputAdapter::new(logical_width, logical_height);
    loop_data
        .events
    ...
    calloop
        ...
        .insert_source(input_backend, move |event, _, data| {
            for event in input_adapter.convert(event) {
...
impl StandaloneInputAdapter {
    fn new(logical_width: f64, logical_height: f64) -> Self {
        Self {
            logical_width,
            logical_height,
    ...
    fn initial_event(&self) -> RawSeatEvent {
        RawSeatEvent::new(
    ...
    fn convert(&mut self, event: InputEvent<LibinputInputBackend>) -> Vec<RawSeatEvent> {
...
mod tests {
    ...
    fn standalone_pointer_starts_centered_and_stays_inside_the_output() {
        let adapter = StandaloneInputAdapter::new(1920.0, 1080.0);
        assert_eq!(
    ...
    fn standalone_finger_scroll_stops_only_the_reported_axis() {
        let mut adapter = StandaloneInputAdapter::new(1920.0, 1080.0);
        let started =
