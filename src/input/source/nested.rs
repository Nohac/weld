...
#[derive(Default)]
pub(crate) struct NestedAdapter {
...
impl NestedAdapter {
    ...
    pub(crate) fn drain(&mut self) -> Drain<'_, RawSeatEvent> {
        self.events.drain(..)
    }

    pub(crate) fn handle_window_event(&mut self, event: WindowEvent, scale_factor: f64, time: u32) {
        match event {
