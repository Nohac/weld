...
const ALLOW_LIST_TYPE : &str = ".*ExternalBuffers.*|.*PRIME.*|.*DRMFormatModifier.*|.*MPEG2.*|.*VP8.*|.*VP9.*|.*H264.*|.*HEVC.*|VACodedBufferSegment|.*AV1.*|VAEncMisc.*|VAProcPipelineParameterBuffer|VASurfaceDecodeMBErrors|VADecodeErrorType";
